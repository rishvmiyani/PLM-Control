...
import {
  ...
  Tag, AlignLeft, AlertTriangle, Info,
} from "lucide-react"

// Matches schema: enum ECOType { PRODUCT BOM ROLLBACK }
const ECO_TYPES: { label: string; value: string }[] = [
  { label: "Product Change", value: "PRODUCT" },
  { label: "BOM Change",     value: "BOM" },
  { label: "Rollback",       value: "ROLLBACK" },
]

// Matches schema: enum RiskLevel { LOW MEDIUM HIGH CRITICAL }
const RISK_LEVELS: { label: string; value: string }[] = [
  { label: "LOW",      value: "LOW" },
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/eco", {
        ...
        body: JSON.stringify({
          title: form.title,
          type: form.type,                     // "PRODUCT" | "BOM" | "ROLLBACK"
          productId: form.productId,
          effectiveDate: form.effectiveDate,
          riskLevel: form.riskLevel,
          versionUpdate: form.createNewVersion,
          proposedChanges: {                   // required Json field in schema
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? `Error ${res.status}: Failed to create ECO`)
        return
      }
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <div style={{ marginBottom: 18 }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: "#4a2d5a", margin: "0 0 4px" }}>
          ECO Details
        </p>
        <div style={{
          ...
          <Info style={{ width: 13, height: 13, color: "#8b3b9e", flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: "#7c5f8a" }}>
            ECOs track all engineering changes applied to a product.
          </span>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px 20px" }}>
        ...
        <div>
          <label style={labelStyle}>
            ...
            ECO Type <span style={{ color: "#e63b6f", marginLeft: 2 }}>*</span>
          </label>
          <select
              ...
              <button
                ...
                style={{
                  ...
                  background: form.riskLevel === r.value
                    ? `${RISK_COLORS[r.value]}18`
                    : "rgba(245,240,252,0.5)",
                  color: form.riskLevel === r.value
                    ? RISK_COLORS[r.value]
                    : "#b0a0bc",
                }}
              >
      ...
      {/* Divider */}
      <div style={{ height: 1, background: "rgba(190,113,209,0.1)", margin: "22px 0 18px" }} />
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button
          ...
          style={{
            ...
            fontSize: 13, fontWeight: 600, fontFamily: font, color: "#fff",
            background: loading
              ? "rgba(139,59,158,0.4)"
              : "linear-gradient(135deg,#8b3b9e,#be71d1)",
            boxShadow: loading ? "none" : "0 4px 14px rgba(139,59,158,0.28)",
            transition: "all 0.2s",
          }}
        >
